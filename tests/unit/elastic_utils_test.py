...
class ElasticUtilsTest(unittest.TestCase):
  ...
  def test_elastic_mode_enabled(self):
    """Tests elastic_mode_enabled."""
    config = mock.MagicMock()
    config.elastic_pause_resume = True
    with mock.patch.object(pathwaysutils, "is_pathways_backend_used", return_value=True):
      self.assertTrue(elastic_utils.elastic_mode_enabled(config))

    config.elastic_pause_resume = False
    with mock.patch.object(pathwaysutils, "is_pathways_backend_used", return_value=True):
      self.assertFalse(elastic_utils.elastic_mode_enabled(config))
  ...
  @mock.patch.object(elastic_utils, "elastic_mode_enabled")
  def test_elastic_pause_resume_disabled(self, mock_enabled):
    """Tests elastic_pause_resume when disabled."""
    mock_enabled.return_value = False
    config = mock.MagicMock()
    decorator = elastic_utils.elastic_pause_resume(config)
  ...
  @mock.patch.object(elastic_utils, "elastic_mode_enabled")
  @mock.patch("pathwaysutils.elastic.manager.Manager")
  def test_elastic_pause_resume_enabled(self, mock_manager, mock_enabled):
    """Tests elastic_pause_resume when enabled."""
    mock_enabled.return_value = True
    config = mock.MagicMock()
    config.checkpoint_dir = "gs://test_bucket/checkpoints"
    config.elastic_max_retries = 3
    config.elastic_retry_interval_seconds = 10
    ...
    elastic_utils.elastic_pause_resume(config)
    ...
    self.assertEqual(kwargs["poll_interval"], 10)
