...
def math_verify_func(items, scores, timeout=300, tmvp_config=None):
...
def preprocess_math_string(text) -> str:
...
def check_numbers(prompts, completions, answer, tmvp_config, **kargs):
  ...
  for gen_idx, (guess, unique_answers) in enumerate(zip(extracted_responses, true_answers)):
    if guess is None or guess == FALLBACK_ANSWER:
      continue

    # Extraction succeeded. Default to incorrect answer penalty until proven correct.
  ...
  if math_verify_queue:
    ...
    scores = math_verify_func(math_verify_queue, scores, tmvp_config=tmvp_config)
...
def extract_answer(response, tmvp_config) -> str:
